...
use crate::{Note, mobile_sync::MobileSyncManager, mobile_config};
...
impl SyncBridge {
    ...
    async fn handle_list_change(manager: &mut MobileSyncManager, title: &str, list: &List, kind: EventKind) -> Result<()> {
        // Convert list to markdown format (same as desktop)
        let markdown_content = Self::list_to_markdown(list)?;
        
        // Use consistent path format (same as desktop would use)
        let logical_path = PathBuf::from(format!("content/lists/{}.md", title.replace('/', "_")));
        
        // Write to temp location but use logical path for sync
        let temp_dir = std::env::temp_dir().join("lst-mobile-sync");
        std::fs::create_dir_all(&temp_dir)?;
        let temp_file = temp_dir.join(format!("{}.md", title.replace('/', "_")));
        std::fs::write(&temp_file, &markdown_content)?;

        // Create event with logical path (for consistent doc ID generation)
        let event = Event {
            kind,
            paths: vec![logical_path.clone()],
            attrs: Default::default(),
        };

        println!("📱 Mobile sync: Processing list '{}' with desktop-compatible path: {}", title, logical_path.display());

        // Trigger sync with the logical path event
        manager.handle_file_event(event).await?;

        Ok(())
    }

    async fn handle_note_change(manager: &mut MobileSyncManager, title: &str, note: &Note, kind: EventKind) -> Result<()> {
        // Notes are already in markdown format
        let content = &note.content;
        
        // Use consistent path format (same as desktop would use)
        let logical_path = PathBuf::from(format!("content/notes/{}.md", title.replace('/', "_")));
        
        // Write to temp location but use logical path for sync
        let temp_dir = std::env::temp_dir().join("lst-mobile-sync");
        std::fs::create_dir_all(&temp_dir)?;
        let temp_file = temp_dir.join(format!("note_{}.md", title.replace('/', "_")));
        ...
        println!("📱 Mobile sync: Processing note '{}' with desktop-compatible path: {}", title, logical_path.display());
    ...
    async fn handle_list_delete(manager: &mut MobileSyncManager, title: &str) -> Result<()> {
        let virtual_path = PathBuf::from(format!("content/lists/{}.md", title));
    ...
    async fn handle_note_delete(manager: &mut MobileSyncManager, title: &str) -> Result<()> {
        let virtual_path = PathBuf::from(format!("content/notes/{}.md", title));

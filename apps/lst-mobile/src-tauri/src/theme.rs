...
mod tests {
    ...
    fn test_default_theme_is_nord() {
        ...
        if let Some(name) = &theme_data.name {
            assert!(
                name.to_lowercase().contains("nord"),
                "Expected theme name to contain 'nord', got: {}",
                name
            );
        }
        ...
        assert!(
            !theme_data.css_variables.is_empty(),
            "CSS variables should not be empty"
        );

        // Verify that the scheme is set correctly
        assert!(!theme_data.scheme.is_empty(), "Scheme should not be empty");
    ...
    fn test_nord_theme_loads_correctly() {
        let theme_data = apply_theme("base16-nord".to_string()).expect("Should load Nord theme");

        // Verify Nord theme properties
        assert!(theme_data.name.is_some(), "Nord theme should have a name");
        assert_eq!(
            theme_data.scheme, "base16-nord",
            "Scheme should be base16-nord"
        );
        assert!(
            !theme_data.css_variables.is_empty(),
    ...
    fn test_theme_list_includes_nord() {
        ...
        assert!(
            themes.contains(&"base16-nord".to_string()),
